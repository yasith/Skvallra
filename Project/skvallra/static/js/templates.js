...
SearchUser = Backbone.Collection.extend({
	...
	url: function() {
		return "/api/v1.0/search/" + this.id + "/users/";
	},
});
...
SearchAction = Backbone.Collection.extend({
	...
	url: function() {
		return "/api/v1.0/search/" + this.id + "/actions/";
	},
});
...
SearchUsersForAction = Backbone.Collection.extend({
	...
	url: function() {
		return "/api/v1.0/search/" + this.id + "/invite_users/";
	},
});
...
ActionUsers = Backbone.Collection.extend({
	...
	url: function() {
		return '/api/v1.0/action_users/' + this.id;
	},
});
...
UserActionInteractions = Backbone.Collection.extend({
	...
	getByUserAndAction: function(user_id, action_id){
		return this.filter(function(elem) {
			return (elem.get("user_id") == user_id) && (elem.get("action_id") == action_id);
		})[0]
	},
	url: function() {
		return '/api/v1.0/useractions/';
	},
});

Setting = Backbone.Model.extend({
	urlRoot: '/api/v1.0/settings/',
});
...
Comment = Backbone.Model.extend({
	urlRoot: '/api/v1.0/comments/',
});

ActionComment = Backbone.Model.extend({
	urlRoot: '/api/v1.0/action_comments/',
});
...
ActionComments = Backbone.Collection.extend({
	...
	url: function() {
		return '/api/v1.0/action_comments/' + this.id;
	},
});
...
RatingAndParticipation = Backbone.Model.extend({
	...
	url: function() {
		return "/api/v1.0/user_actions/" + this.actionId + "/get_useraction";
	}
});

Suggested = Backbone.Collection.extend({
	url: function () {
		return '/api/v1.0/suggested/';
	}
});
...
SettingsView = Backbone.View.extend({
	...
	render: function() {
		...
		var editOnHover = function () {
			...
			var divImg = $("<div class='edit pull-right'><img src='/static/images/edit.png' id='edit_img'></div>");
			$(this).append(divImg);
...
ActionListView = Backbone.View.extend({
	...
	render_image: function() {
		$(this.collection.models).each(function() {
			...
			imageView.$el = $('#' + this.attributes.id + '.actionimage');			
			image.fetch();
		});
	},
	navi: function(event) {
...
SearchUserView = Backbone.View.extend({
	...
	render: function() {
		...
		$(temp).each(function (index, element) {
			$.ajax({
				url: '/api/v1.0/users/' + element.id + '/isfriend',
				type: 'GET',
				dataType: 'json',
				async: false,
			})
...
SearchUsersForActionView = SearchUserView.extend({
	...
	add_user: function (event) {
		...
		var userAction = new UserActionInteraction({ "action_id": this.id, "user_id": userId, "role": $.app._participant });
		userAction.save({}, {
			success: function() {
...
SearchActionView = Backbone.View.extend({
	...
	render: function() {
		...
		$(temp).each(function (index, element) {
			$.ajax({
				url: '/api/v1.0/user_actions/' + element.id + '/get_useraction',
				type: 'GET',
				dataType: 'json',
				async: false,
			})
	...
	render_image: function() {
		$(this.collection.models).each(function() {
			var image = new Images({id: this.attributes.image});
			var imageView = new ImageView({model: image});
			imageView.$el = $('#' + this.attributes.id + '.actionsearchimage');
			image.fetch();
		});
	},
	navi: function(event) {
	...
	join_action: function (event) {
		...
		var userAction = new UserActionInteraction({'user_id': user_id, 'action_id': action_id, 'role': $.app._participant});
		userAction.save();
		this.collection.fetch();
	},
	leave_action: function (event) {
		...
		$.ajax({
			url: '/api/v1.0/user_actions/' + id + '/get_useraction',
			type: 'GET',
			dataType: 'json',
			async: false,
		})
...
ProfileView = Backbone.View.extend({
	...
	render: function() {
		...
		if (!temp.me) {
			$.ajax({
				url: '/api/v1.0/users/' + this.model.get('id') + '/isfriend',
				type: 'GET',
				dataType: 'json',
				async: false,
			})
		...
		$(document).ready(function() {
			if (model.get('id') === $.app.user.get('id')) {
				$('.editable').hover(function() {
					$(this).append("<div class='edit' style='display: inline-block;'><img src='/static/images/edit.png' id='edit_img'></div>");
					$(this).children('.edit').click(function (event) {
						var parent = $(this).parent();
						$(this).remove();
						parent.html("<input type='text' id='editing' style='width: 100%;' value='" + $.trim(parent.text()) + "'/>");
			...
			success: function () {
				if (model.get('id') === $.app.user.get('id')) {
					...
					$('#profileimagebox').hover(function() {
						$(this).append("<div class='edit'><img src='/static/images/edit.png' id='edit_img'></div>");
	...
	render_rating: function (rating) {
		$('.rating').raty({ 
			path: '/static/images/raty-img', // icons directory
...
ActionMainView = Backbone.View.extend({
	...
	edit_image: function() {
		...
		$('#actionpageimage').hover(function() {
			...
			$(this).append("<div class='edit pull-right'><img src='/static/images/edit.png' id='edit_img'></div>");
	...
	add_users: function () {
		$('.users-editable').hover(function () {
			...
			$(this).children(".edit").remove();
			var divIcon = $("<div class='edit pull-right'><img src='/static/images/plus_small.png' id='edit_img'></div>");
			$(this).append(divIcon);		
	...
	set_lock_icon: function(actionStatus) {
		...
		var lockIcon;
		if (actionStatus) {
			lockIcon = $('<img src="/static/images/unlock_small.png" class="unlock" alt="unlock" />');
		} else {
			lockIcon = $('<img src="/static/images/lock_small.png" class="lock" alt="lock" />');
		}
...
ActionControlsView = Backbone.View.extend({
	...
	leave_action: function() {
		...
		this.model.url = "/api/v1.0/useractions/" + this.model.get('id');
		this.model.destroy({
	...
	join_action: function() {
		...
		var UserAction = new UserActionInteraction({
			"action_id": $.app.actionView.model.get('id'), 
			"user_id": $.app.user.id,  
	...
	draw_rating: function(score) {
		...
		$('.rating').raty({ 
			path: '/static/images/', // icons directory
			hints: ["very bad", "poor", "okay", "good", "excellent"], // star hints
...
ActionParticipantsView = ActionFriendListView.extend({
	...
	remove_users: function() {
		var editOnHover = function () {
			...
			var divIcon = $("<div class='remove_user'><img src='/static/images/cross_small.png' id='edit_img'></div>");
...
ActionView = Backbone.View.extend({
	...
	render_comments: function() {
		var comments = new ActionComments([], {id: this.model.get('id')}); // create collection with id 
	...
	render_action_controls: function() {
		var lock = new RatingAndParticipation([], {id: this.model.get('id')});
	...
	render_rating_and_participation: function() {
		var actionStatusBar = new RatingAndParticipation([], {id: this.model.get('id')});
	...
	create_action: function() {
		...
		newAction.model.save({}, {
			success: function(model, response, options)	{
				...
				var userAction = new UserActionInteraction({
										"action_id": newAction.model.get('id'), 
										"user_id": $.app.user.id,  
										"role": $.app._organizer });
				userAction.save({}, {
					success: function() {
						router.navigate("/action/" + newAction.model.get('id'), {trigger: true});
					},
					error: function(model, response, options) {
						// set an id so a model is considered 'old' by Backbone so destroy() can be fired
						newAction.model.set('id', newAction.model.get('id'));
...
$.app.authenticate = function() {
	...
	$.ajax({
		...
		$('#navbar-form').after('<ul id="links" class="nav navbar-nav pull-right"><li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><img src="/static/images/other_gear_white_small.png" /></a><ul class="dropdown-menu"><li><a id="changepw" href="#">Change Password</a></li></ul></li></ul>');
		$("#changepw").click($.app.changepw);
		$("#links").find('ul').prepend('<li id="configure_action"><a href="/configure_action">Create Action</a></li>');
		$("#links").prepend('<li><a href="/suggested"><img src="/static/images/friends_small.png" /></a></li>');
		$.ajax({
			url: '/is_admin',
			type: 'GET',
			dataType: 'json',
